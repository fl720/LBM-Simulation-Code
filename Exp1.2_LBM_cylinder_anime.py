...
figure= plt.figure(figsize=(4, 2), dpi = 320)
fluidImage = plt.imshow(np.ones([y, x]), origin='lower', norm=plt.Normalize(-.1,.1), cmap=plt.get_cmap('bwr'), interpolation='none')
...
animate = matplotlib.animation.FuncAnimation(figure, nextFrame, frames =range(1000), interval = 1)
#plt.show()


writer = matplotlib.animation.writers['ffmpeg']
writer = writer(fps=24 )
animate.save('LBM_CYLINDER.mp4', writer=writer)
#plt.close(fig) 
